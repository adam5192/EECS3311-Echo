...
class DataLog extends Log {
	...
	public void setLogHeight(double height) {
		try {
			if (height < 0.0)
				throw new InvalidAttributeValueException("Height cannot be negative.");
	...
	public void setLogWeight(double weight) {
		try {
			if (weight < 0.0)
				throw new InvalidAttributeValueException("Weight cannot be negative.");
...
class MealLog extends Log {
	 ...
	 * 
	 * @return the meal type associated with the log, null if the value of mealType
	 *         is invalid.
	 */
	...
	public int calculateCalories() {
		return ingredients.stream().mapToInt(Ingredient::getCalories).sum(); // converts ingredients list to stream and
																				// gets calories of each ingredient
	}
...
class Ingredient {
	...
	@Override
	public String toString() {
		return "Ingredient{" + "name= " + name + '\'' + ", calories = " + calories + "cals, fat = " + fat
				+ "g, protein = " + protein + "g, carbs = " + carbs + "g}";
	}
...
class ExerciseLog extends Log {
	...
	public void setCaloBurnt(int caloBurnt) {
		try {
			if (caloBurnt < 0)
				throw new InvalidAttributeValueException("The value of calorie burnt cannot be negative.");
			this.caloBurnt = caloBurnt;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void setTime(double time) {
		try {
			if (time < 0.0)
				throw new InvalidAttributeValueException("Time spent exercising cannot be negative.");
			this.time = time;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// Getters
	public int getCaloBurnt() {
		return this.caloBurnt;
	}
